...
// Upstream rotated CDN + version in 2026:
//   assets.lk21.party/?v=4 → assets.showcdnx.com/?v=8 → assets.lk21.party/?v=15.
// When they rotate again, update these two URLs (and bump the cache key if needed).
const PLAYER_JS_URL = 'https://assets.lk21.party/js/player.js?v=15';
const SCRIPT_JS_URL = 'https://assets.lk21.party/js/script.js?t=movie&v=15';
const SOURCE_TTL    = 6 * 3600; // seconds — refresh upstream scripts every 6h

// In v15 the `_L` decrypt function is created inside an async Promise chain at
// the end of player.js — it doesn't exist immediately after eval() returns.
// Wait this long for microtasks/timers to drain before looking it up.
...
// Find the decrypt function by behavior. v4/v8/v15 all expose it as
// `globalThis._L`, so that's the fast path. v15 ALSO assigns the same
// function to a rotating `awcbb_yhh_funN` global (N varies per build) — we
// scan top-level functions as a fallback in case the `_L` alias gets dropped
// in a future rotation. Pre-v8 had a `vmX_<hex>` namespace style; the scan
// covers any plain top-level function so we're forward-compatible.
function discoverDecryptFn(window, sentinelToken) {
  const looksRight = (out) => typeof out === 'string' && /^https?:\/\//.test(out);

  // Fast path: globalThis._L
  ...
  // Fallback: scan all own-property functions on window. Skip jsdom internals
  // (those start with `_` followed by uppercase, or are bound DOM methods).
...
async function buildEnvironment(sentinelToken) {
  ...
  // v15 setup is async — _L is created inside a Promise.then() that runs
  // after eval() returns. Let microtasks + a short timer drain before lookup.
