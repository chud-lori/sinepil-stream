  ...
  $('script[type="application/ld+json"]').each((_, el) => {
    try {
      ...
      for (const n of nodes) {
        ...
        if (types.includes('TVSeries')) hasTvSeries = true;
        if (types.some(t => ['Movie', 'TVSeries', 'VideoObject'].includes(t))) {
          result = { ...result, ...n };
        }
      }
    } catch (_) {}
  });
  result._hasTvSeries = hasTvSeries;
  return result;
}
...
function isNetworkError(e) {
  return !e.response || e.code === 'ECONNREFUSED' || e.code === 'ENOTFOUND'
...
// Does this look like a page served by the source site at all? A real listing
// page keeps its nav/footer chrome even when the genre has no results; a dead
// mirror's redirect interstitial has neither. Without this check, an
// interstitial (HTTP 200, zero cards) is indistinguishable from an empty genre
// and gets cached as an empty rail.
function looksLikeSourcePage($) {
  return $('nav').length > 0 || $('footer, .footer').length > 0;
}

async function getMovie(slug) {
  // Upstream changed routing in 2026-05: /slug/ now 404s, /slug returns 200
  // (and internally rewrites to ?page=slug). Keep the no-slash form.
  ...
  if (finalHost && finalHost !== baseHost && host.isKnownMovieHost(finalHost)) {
  ...
  if (ld._hasTvSeries || ld['@type'] === 'TVSeries') {
    movieStmts.delete.run(slug);
  ...
  if (rawPlayers.length === 0) {
    const src = $('#main-player').attr('src') || $('#main-player iframe, .main-player iframe').first().attr('src') || '';
...
// Fetch a listing page (homepage / genre / year) with host failover, adopting
// whatever host the site redirects us to. `browse()` used to call fetchHtml
// directly, so a stuck-on-a-dead-mirror activeHost produced empty rails
...
async function fetchListing(path) {
  const attempt = async () => {
    ...
    const finalHost = (() => { try { return new URL(finalUrl).hostname; } catch { return ''; } })();
    if (finalHost && finalHost !== host.currentHost() && host.isKnownMovieHost(finalHost)) {
      host.adoptHost(finalHost);
    }
    return $;
  };
  ...
  // 200 but not actually the source site — rotate and retry once before
  // believing it.
  if (!looksLikeSourcePage($)) {
    host.rotateAfterFailure();
    $ = await attempt();
    if (!looksLikeSourcePage($)) {
      const err = new Error('Movie source returned an unrecognised page');
      err.status = 502;
      throw err;
    }
  }
...
async function seedIndex() {
  ...
  for (const p of seedPages) {
    try {
      indexMovies(extractCards(await fetchListing(p)));
...
function slugFromSourceUrl(url) {
  try {
    ...
    if (!host.isKnownMovieHost(hostname)) return null;
