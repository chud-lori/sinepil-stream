...
// Listing pages go through fetchWithFailover (not bare fetchHtml) so a dead
// mirror rotates instead of yielding empty rails forever.
async function browse(path = '') {
  const clean = path.replace(/^\/|\/$/g, '');
  const res = await fetchWithFailover(clean ? `/${clean}` : '/', {
    headers: DEFAULT_HEADERS,
    timeout: 20000,
    maxRedirects: 5,
  });
