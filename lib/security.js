...
// Poster CDNs we re-serve through /api/img. Separate from the embed allowlist:
// these are image origins only, and the reason we proxy them at all is that
// some client networks (ISP-level domain blocks) can't reach them directly
// even though our server can.
const POSTER_HOST_ALLOWLIST = [
  'showcdnx.com',      // cover.showcdnx.com, poster.showcdnx.com
  'lk21.party',        // static-jpg.lk21.party
  'lk21official.cc',
  'nontondrama.my',
];
...
function isAllowedPosterHost(hostname) {
  const h = (hostname || '').toLowerCase();
  return POSTER_HOST_ALLOWLIST.some(allowed => h === allowed || h.endsWith('.' + allowed));
}
...
async function assertSafeUrl(rawUrl, hostCheck) {
  let u;
...
async function assertSafeOutboundUrl(rawUrl) {
  return assertSafeUrl(rawUrl, isAllowedEmbedHost);
}

async function assertSafePosterUrl(rawUrl) {
  return assertSafeUrl(rawUrl, isAllowedPosterHost);
}
